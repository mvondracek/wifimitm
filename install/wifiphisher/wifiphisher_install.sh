#!/usr/bin/env bash
##
## Install wifiphisher and setup its virtualenv
##
...
function main()
{
    ...
    announce_task "${TASK}"
    git clone https://github.com/mvondracek/wifiphisher.git
    check_task_result $? "${TASK}"


    TASK="cd ${INSTALL_DIR}/wifiphisher"
    announce_task "${TASK}"
    cd ${INSTALL_DIR}/wifiphisher
    check_task_result $? "${TASK}"


    TASK="apply patch"
    announce_task "${TASK}"
    patch -p1 < ${SCRIPT_DIR}/wifiphisher.patch
    check_task_result $? "${TASK}"
